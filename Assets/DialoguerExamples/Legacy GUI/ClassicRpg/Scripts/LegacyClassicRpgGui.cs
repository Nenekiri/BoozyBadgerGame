...
public class LegacyClassicRpgGui : MonoBehaviour
{
	...
	void OnGUI()
	{
		...
		if (_theme == "good")
		{
			drawDialogueBox(dialogueBoxRect, new Color(0.824f, 0.706f, 0.549f)); //old colors were 0.2f, 0.8f, 0.4f
		}
		else if (_theme == "bad")
		{
		...
		{
			Rect nameBoxRect = new Rect(dialogueBoxRect.x + 350, dialogueBoxRect.y - 60, 150 * _windowTweenValue, 50 * _windowTweenValue);
        ...
        //Draw Portrait box
        if (_portrait != string.Empty)
        {
            Rect portraitBoxRect = new Rect(dialogueBoxRect.x, dialogueBoxRect.y - 210, 150 * _windowTweenValue, 50 * _windowTweenValue);
            portraitBoxRect.width = Mathf.Clamp(portraitBoxRect.width, 32, 2000);
            portraitBoxRect.height = Mathf.Clamp(portraitBoxRect.height, 32, 2000);
            portraitBoxRect.width = 200;
            portraitBoxRect.height = 200; 
            drawDialogueBox(portraitBoxRect);
            testTexture = Resources.Load(_portrait) as Texture;
            drawShadowedTexture(new Rect(portraitBoxRect.x + (15 * _windowTweenValue) - (5 * (1 - _windowTweenValue)), portraitBoxRect.y + (5 * _windowTweenValue) - (10 * (1 - _windowTweenValue)), portraitBoxRect.width - (30 * _windowTweenValue), portraitBoxRect.height - (5 * _windowTweenValue)), testTexture);
        }
        ...
        //trying to add in the code for the portrait stuff
        //testTexture = Resources.Load(_portrait) as Texture;
        //GUI.Label(new Rect(350, 250, testTexture.width, testTexture.height), testTexture);
	...
	{
		drawDialogueBox(rect, new Color(245 / 255.0F, 222f / 255.0F, 179 / 255.0F)); //default values are 45 / 255.0F, 111f / 255.0F, 255.0F, 255 / 255.0F
    }
    ...
    private void drawShadowedTexture(Rect rect, Texture tex)
    {
        GUI.color = new Color(0, 0, 0, 0.5f);
        GUI.Label(new Rect(rect.x + 1, rect.y + 2, rect.width, rect.height), tex);
        GUI.color = GUI.contentColor;
        GUI.Label(rect, tex);
    }
